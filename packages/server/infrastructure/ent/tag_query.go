...
func (tq *TagQuery) QueryThreads() *ThreadQuery {
...
func (tq *TagQuery) QueryThreadTags() *ThreadTagQuery {
...
func (tq *TagQuery) WithThreads(opts ...func(*ThreadQuery)) *TagQuery {
	query := (&ThreadClient{config: tq.config}).Query()
...
func (tq *TagQuery) WithThreadTags(opts ...func(*ThreadTagQuery)) *TagQuery {
	query := (&ThreadTagClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withThreadTags; query != nil {
		if err := tq.loadThreadTags(ctx, query, nodes,
			func(n *Tag) { n.Edges.ThreadTags = []*ThreadTag{} },
			func(n *Tag, e *ThreadTag) { n.Edges.ThreadTags = append(n.Edges.ThreadTags, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadThreads(ctx context.Context, query *ThreadQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Thread)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(thread.FieldID), joinT.C(tag.ThreadsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.ThreadsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.ThreadsPrimaryKey[1]))
...
func (tq *TagQuery) loadThreadTags(ctx context.Context, query *ThreadTagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *ThreadTag)) error {
