...
type ThreadEdges struct {
	...
	Tags []*ThreadTag `json:"tags,omitempty"`
...
func (e ThreadEdges) CommentsOrErr() ([]*Comment, error) {
	if e.loadedTypes[2] {
...
// was not loaded in eager-loading.
func (e ThreadEdges) TagsOrErr() ([]*ThreadTag, error) {
	if e.loadedTypes[3] {
		return e.Tags, nil
	}
...
// was not loaded in eager-loading.
func (e ThreadEdges) ThreadTaggingsOrErr() ([]*ThreadTagging, error) {
	if e.loadedTypes[6] {
...
func (*Thread) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case thread.FieldIsAutoGenerated, thread.FieldIsNotifyOnComment:
			values[i] = new(sql.NullBool)
		case thread.FieldID, thread.FieldBoardId, thread.FieldUserId:
			values[i] = new(sql.NullInt64)
		case thread.FieldTitle, thread.FieldDescription, thread.FieldThumbnailUrl, thread.FieldIPAddress, thread.FieldStatus:
...
// QueryThreadTaggings queries the "thread_taggings" edge of the Thread entity.
func (t *Thread) QueryThreadTaggings() *ThreadTaggingQuery {
	return NewThreadClient(t.config).QueryThreadTaggings(t)
}
