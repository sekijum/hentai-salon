...
func (tc *ThreadCreate) SetIsAutoGenerated(b bool) *ThreadCreate {
	tc.mutation.SetIsAutoGenerated(b)
	return tc
}

// SetNillableIsAutoGenerated sets the "isAutoGenerated" field if the given value is not nil.
func (tc *ThreadCreate) SetNillableIsAutoGenerated(b *bool) *ThreadCreate {
	if b != nil {
...
func (tc *ThreadCreate) AddComments(c ...*Comment) *ThreadCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
...
func (tc *ThreadCreate) AddTags(t ...*ThreadTag) *ThreadCreate {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
...
func (tc *ThreadCreate) defaults() {
	if _, ok := tc.mutation.IsAutoGenerated(); !ok {
		v := thread.DefaultIsAutoGenerated
...
func (tc *ThreadCreate) check() error {
	...
	if _, ok := tc.mutation.IsAutoGenerated(); !ok {
		return &ValidationError{Name: "isAutoGenerated", err: errors.New(`ent: missing required field "Thread.isAutoGenerated"`)}
	}
	if _, ok := tc.mutation.IsNotifyOnComment(); !ok {
